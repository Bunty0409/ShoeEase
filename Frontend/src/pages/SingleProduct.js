...
const SingleProduct = () => {
  ...
  const nextImage = () => {
    ...
    if (len === 0) return;
    setMainImageIndex((idx) => (idx + 1) % len);
  };
  const prevImage = () => {
    const len = productState?.images?.length || 0;
    if (len === 0) return;
    setMainImageIndex((idx) => (idx - 1 + len) % len);
  };

  const thumbScroll = (dir = "next") => {
      ...
      <Container class1="description-wrapper py-5 home-wrapper-2">
        <div className="row">
          <div className="col-12">
            <h4>Description</h4>
            <div className="p-3" style={{ background: "var(--glass-bg)", borderRadius: "16px", border: "1px solid var(--glass-border)", backdropFilter: "blur(12px)" }}>
              <p
                className="desc-html"
